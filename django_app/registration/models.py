...
class RegistrationManager(models.Manager):
    ...
    def create_profile(self, user):
        ...
        """
        # hold a random variable
        random_var = str(random.random())
        # generate a new encryption algorithm 
        new_sha = sha.new('SHA1')
        # update the algorithm with the random variable we just created
        new_sha.update(random_var)
        # um, whatever... (don't add pepper!)
        salt = new_sha.hexdigest()[:5]
        # create new encryption algorithm as a temp key
        temp_activation_key = sha.new('SHA1')
        # update that key...
        temp_activation_key.update(salt+user.username)
        # generate the final key to be used in the email
        activation_key = temp_activation_key.hexdigest()
